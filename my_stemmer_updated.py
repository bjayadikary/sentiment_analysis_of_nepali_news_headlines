...
def process_stemming_of_word(shabdakosh_words_list, arranged_suffix_dictionary, input_word):
    ...
    start_from_initial = 1
    while start_from_initial == 1 and (input_word not in shabdakosh_words_list):
        previous = input_word
        for suffix_len in suffix_len_keys:
            if (len(input_word) - suffix_len) >= 2:
                for suffix in arranged_suffix_dictionary[suffix_len]:
                    if input_word.endswith(suffix):
                        input_word = input_word[:-suffix_len]
                        break

                if input_word == previous:
                    continue
                else:
                    break
            else:
...
def stem(shabdakosh_words_list, arranged_suffix_dictionary, input_string):
    ...
    for each_word in inputs.split():
        if each_word not in [suffix for suffix_list in arranged_suffix_dictionary.values() for suffix in suffix_list]:
...
def stem_it(raw_string):
    files_for_stemming_path = os.getcwd() + '/data_files/files_for_stemming'
